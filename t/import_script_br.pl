...
use FindBin '$Bin';
use lib "$Bin/../lib";
use POFOMD::Script::Import::BR;
use DateTime;

diag(q{DON'T RUN THIS ON THE LIVE SERVER!});

{
    my $script = POFOMD::Script::Import::BR->new(
        application_name => 'POFOMD',
        dataset          => "/non/existing/path/to/csv",
    );
    isa_ok($script, 'POFOMD::Script::Import::BR');
...

{
    ...
    is($gasto_rs->count({ dataset_id => $dataset_id }), 9, '9 expenses in this dataset in the DB');
