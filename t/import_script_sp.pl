...

{
    ...
    isa_ok($script->csv_obj, 'Text::CSV_XS', 'the csv object');
    isa_ok($script->text2uri, 'Text2URI', 'the Text2URI object');
    is($script->dataset, "$Bin/data/sp.sample.csv", "Dataset file is correctly set");
    is($script->year, 1500, "Year is correctly set");

    my $periodo_rs = $script->schema->resultset('Periodo');
    my $gasto_rs = $script->schema->resultset('Gasto');

    is($periodo_rs->count({ ano => 1500 }), 0, 'no year 1500 in the DB');
    ok(my $dataset_id = $script->dataset_id, 'got dataset id');
